...
| [Perf] | @perf | Rex | Performance |
| [Sec] | @sec | Vex | Security |
| [Inf] | @infra | Io | Infrastructure |
...
2. **Team (default)** Orchestrator routes to best agent. Agent calls peers dynamically.
3. **Select** `/apex select kai,rex` — Only those active.
...
- `apex-mirage <command>` — Execute across virtual filesystem backends
...
@toolName                # Invoke connected tool (e.g. @gmail, @github)
```
