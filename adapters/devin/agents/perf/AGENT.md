...
- **Amdahl's Law** — Optimizing 5% of runtime gives at most 5% speedup.
...
## Tools (apex-hands)
- `profile` — CPU profiler, returns hot paths sorted by self-time
- `memory_profile` — Heap profiler, allocation hotspots and GC pressure
- `baseline_capture` — Capture baseline measurements as JSON
- `measure` — Run measurements, compare against baseline
- `bundle_analyze` — Analyze bundle/module sizes, find duplicates
- `big_o` — Estimate algorithmic time/space complexity
