...
export const AppDetailsView: React.FC<AppDetailsViewProps> = ({ tool, onBack, onClose }) => {
  ...
  const toolBanners = tool.bannerImages.length > 0 ? tool.bannerImages : [];
    ...
    <div className="composio-app-details">
      ...
      <div className="app-details-body">
        ...
        <div className="app-details-hero">
          ...
          {/* Connect button */}
          <button
            className={`app-details-connect-btn ${connected ? 'connected' : ''}`}
        ...
        {/* Banner carousel */}
        {toolBanners.length > 0 && (
          <div className="app-details-banners-section">
            <h3 className="app-details-section-title">Preview</h3>
            <div className="app-details-banner-carousel">
              {toolBanners.length > 1 && bannerIndex > 0 && (
                <button
                  className="banner-nav-btn banner-nav-prev"
                  onClick={() => setBannerIndex(i => Math.max(0, i - 1))}
                >
                  <ChevronLeft size={20} />
                </button>
              )}
              <div className="banner-carousel-track" style={{ transform: `translateX(-${bannerIndex * 100}%)` }}>
                {toolBanners.map((img, i) => (
                  <div key={i} className="banner-slide">
                    <img src={img} alt={`${tool.name} preview ${i + 1}`} />
                  </div>
                ))}
              </div>
              {toolBanners.length > 1 && bannerIndex < toolBanners.length - 1 && (
                <button
                  className="banner-nav-btn banner-nav-next"
                  onClick={() => setBannerIndex(i => Math.min(toolBanners.length - 1, i + 1))}
                >
                  <ChevronRight size={20} />
                </button>
              )}
              {toolBanners.length > 1 && (
